...
public class Magician : Character {
    ...
    void ProcessInputController() {
        if (!blockedInput) { // Only process movement if not Unconscious
            ...
            else if (controller.RightStickY > 0) {
                directions[0] = false;
                directions[1] = false;
                directions[2] = true;
                directions[3] = false;
    ...
    void ProcessMovement() {
        if (blockedInput) {
            ...
            if (this.gameObject.GetComponentInChildren<Camera>() != null) { 
                ...
                this.GetComponentInChildren<Camera>().transform.position = new Vector3(.23f, 2, -15);
            }
        }
        if (!blockedInput) {
            ...
            if (directions[2] && holding_item) {
                held_item.transform.eulerAngles = new Vector3(0, 5f);
            }
            else if (directions[3] && holding_item) {
                held_item.transform.eulerAngles = new Vector3(0, -5f);
            }
            if (in_closet || in_box)
                rb.velocity = Vector3.zero;
